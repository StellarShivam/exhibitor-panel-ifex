...
import { useGetAttendees } from 'src/api/inauguralAttendee';
...
import { IAttendee } from 'src/types/inauguralAttendee';
...
const TABLE_HEAD = [
  // { id: 'profile', label: 'Aadhaar Card' },
  { id: 'name', label: 'Attendee' },
...
export default function UserListView() {
  ...
  const { attendees, attendeesLoading, reFetchAttendees } = useGetAttendees(
    eventData?.state.exhibitorId
  );

  console.log('attendees', attendees);

  const [tableData, setTableData] = useState<IAttendee[]>([]);
  ...
  const handleEditRow = useCallback(
    (id: number) => {
      router.push(paths.dashboard.teamManagement.edit(id));
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'VVIP Registration', href: paths.dashboard.root },
            { name: 'Attendees' },
          ]}
          action={
            <Button
        ...
        >
          <Stack direction="row" alignItems="start" spacing={1}>
            ...
            <Typography variant="subtitle2" sx={{ color: 'info.main' }}>
              ...
              <strong>
                {
                  events.find((event) => event.eventId === eventData.state.eventId)
                    ?.totalNomineeCount
                }
              </strong>
            </Typography>
...
function applyFilter({
  ...
  //   inputData = inputData.filter((user) => permissions.includes(user.permissions));
  // }
