...
import { generateMultiReceipt, generateReceipt, updatePaymentDetails } from 'src/api/payment-summary';
...
export const TransactionTableRow: React.FC<Props> = ({ row, refetch }) => {
  ...
  return (
    <TableRow>
      ...
      <SlimTableCell>{row.data?.transactionDate ? fDate(row.data?.transactionDate) : fDate(row.createdAt)}</SlimTableCell>
      <SlimTableCell>
        {row.formName}
      </SlimTableCell>
      <SlimTableCell sx={{ textTransform: 'capitalize' }}>
          {row.paymentMode}
      </SlimTableCell>
